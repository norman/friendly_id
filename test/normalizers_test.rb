...
# Pins each normalizer's output, so that an upgrade to Active Support or babosa
# fails the build rather than silently rewriting applications' URLs.
class NormalizersTest < TestCaseClass
  ...
  # separator, which is why the Active Record adapter cannot switch silently.
  DIVERGENT = {
  ...
  if defined?(Babosa)
    ...
    # The reason the default is wider than [:latin]: without them each produces
    # an empty string and the adapter falls back to a UUID.
    ...
    # a Latin result. This is what lets the default set grow safely.
...
# The tests above exercise the normalizer objects in isolation, and would all
# still pass if no model ever reached one. These cover the wiring.
class SluggedNormalizerTest < TestCaseClass
  ...
  # The guarantee that upgrading to 6.0 rewrites nobody's URLs.
  ...
  # so normalizer choice is provably not inferred from the bundle.
