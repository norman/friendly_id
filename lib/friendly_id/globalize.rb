    ...
    module FinderMethods
      ...
      def find_one(id)
        ...
        unless respond_to?('translated?')
          ...
          joins(:translations).where("#{@klass}_translations.locale = ? AND #{@klass}_translations.#{@klass.friendly_id_config.query_field} = ?", I18n.locale, id).first or
          # if locale is not translated fallback to default locale
          joins(:translations).where("#{@klass}_translations.locale = ? AND #{@klass}_translations.#{@klass.friendly_id_config.query_field} = ?", I18n.default_locale, id).first or
          super
        end
      end
