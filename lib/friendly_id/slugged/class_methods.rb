  ...
  module Slugged
    module ClassMethods
      ...
      def find_one(id_or_name, options) #:nodoc:#
        ...
        name, sequence = FriendlyId.parse_friendly_id(id_or_name)
        ...
        if result
          result.friendly_id_finder = Finder.new(:model => result, :name => id_or_name)
      ...
      def assign_finders(slugs, results) #:nodoc:#
        slugs.each do |slug|
          results.select { |r| r.id == slug.sluggable_id }.each do |result|
            result.friendly_id_finder = Finder.new(:model => result, :slug => slug)
          end
        end
      end
