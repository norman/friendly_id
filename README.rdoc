...
Checking the slugs table all the time can be a big performance drain, so as of
2.3.0, FriendlyId offers slug caching to improve performance.
...
Important: if you activate this option on a table with already existing slugs,
do not forget to redo the slugs:
...
We are grateful for many contributions from the Ruby and Rails community.
